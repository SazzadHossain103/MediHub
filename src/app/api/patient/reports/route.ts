import mongoose from "mongoose"
import { NextRequest, NextResponse } from "next/server"
import jwt from "jsonwebtoken"
import dbConnect from "@/src/lib/dbConnect"
import Patient from "@/src/models/patientModel"
import cloudinary from "@/src/lib/cloudinary"

interface TokenData {
  sub: string
  role: string
  email?: string
}

const verifyAuth = (authHeader: string | null) => {
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    throw new Error("Authorization token missing")
  }

  const token = authHeader.split(" ")[1]
  return jwt.verify(token, process.env.JWT_SECRET!) as TokenData
}

const uploadReportFile = async (fileData: string, fileName: string) => {
  const match = fileData.match(/^data:(.*?);base64,(.+)$/)
  if (!match) {
    throw new Error("Invalid file data")
  }

  const buffer = Buffer.from(match[2], "base64")
  const mimeType = match[1]

  const result: any = await new Promise((resolve, reject) => {
    cloudinary.uploader
      .upload_stream(
        {
          folder: "medihub/patient/reports",
          resource_type: "auto",
          public_id: `${Date.now()}-${fileName}`,
        },
        (error, result) => {
          if (error) reject(error)
          else resolve(result)
        }
      )
      .end(buffer)
  })

  return result.secure_url
}

// GET all reports for a patient
export async function GET(req: NextRequest) {
  try {
    await dbConnect()

    const authHeader = req.headers.get("authorization")
    const decoded = verifyAuth(authHeader)

    if (decoded.role !== "patient" && decoded.role !== "admin" && decoded.role !== "super_admin") {
      return NextResponse.json({ success: false, message: "Unauthorized access" }, { status: 403 })
    }

    let patientId = decoded.sub
    if (decoded.role === "admin" || decoded.role === "super_admin") {
      const url = new URL(req.url)
      const userId = url.searchParams.get("userId")
      if (userId) patientId = userId
    }

    const patient = await Patient.findOne({ userId: patientId }).lean()
    if (!patient) {
      return NextResponse.json({ success: false, message: "Patient profile not found" }, { status: 404 })
    }

    const reports = patient.reports || []
    const sortedReports = reports.sort((a: any, b: any) => {
      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    })

    return NextResponse.json({ success: true, reports: sortedReports }, { status: 200 })
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error.message }, { status: 500 })
  }
}

// POST create a new report
export async function POST(req: NextRequest) {
  try {
    await dbConnect()

    const authHeader = req.headers.get("authorization")
    const decoded = verifyAuth(authHeader)

    if (decoded.role !== "patient") {
      return NextResponse.json({ success: false, message: "Unauthorized access" }, { status: 403 })
    }

    const body = await req.json()
    const { name, category, date, lab, doctor, status, fileData, fileName, results, notes } = body

    if (!name || !category || !date || !fileData) {
      return NextResponse.json(
        { success: false, message: "Name, category, date, and file are required" },
        { status: 400 }
      )
    }

    const patientExists = await Patient.exists({ userId: decoded.sub })
    if (!patientExists) {
      return NextResponse.json({ success: false, message: "Patient profile not found" }, { status: 404 })
    }

    // Upload file to Cloudinary
    const fileUrl = await uploadReportFile(fileData, fileName || `report-${Date.now()}`)

    const newReport = {
      _id: new mongoose.Types.ObjectId(),
      name,
      category,
      date,
      lab: lab || null,
      doctor: doctor || null,
      status: status || "ready",
      fileUrl,
      results: results || [],
      notes: notes || null,
      createdAt: new Date(),
      updatedAt: new Date(),
    }

    await Patient.findOneAndUpdate(
      { userId: decoded.sub },
      { $push: { reports: { $each: [newReport], $position: 0 } } },
      { new: true }
    )

    return NextResponse.json({ success: true, report: newReport }, { status: 201 })
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error.message }, { status: 500 })
  }
}

// PATCH update a report
export async function PATCH(req: NextRequest) {
  try {
    await dbConnect()

    const authHeader = req.headers.get("authorization")
    const decoded = verifyAuth(authHeader)

    if (decoded.role !== "patient") {
      return NextResponse.json({ success: false, message: "Unauthorized access" }, { status: 403 })
    }

    const url = new URL(req.url)
    const reportId = url.searchParams.get("id")

    if (!reportId) {
      return NextResponse.json({ success: false, message: "Report ID is required" }, { status: 400 })
    }

    const body = await req.json()
    const { name, category, date, lab, doctor, status, results, notes, fileData, fileName } = body

    const patient = await Patient.findOne({ userId: decoded.sub })
    if (!patient) {
      return NextResponse.json({ success: false, message: "Patient profile not found" }, { status: 404 })
    }

    const reportIndex = patient.reports.findIndex((r: any) => r._id.toString() === reportId)
    if (reportIndex === -1) {
      return NextResponse.json({ success: false, message: "Report not found" }, { status: 404 })
    }

    const report = patient.reports[reportIndex]

    // Update file if new file is provided
    let fileUrl = report.fileUrl
    if (fileData) {
      fileUrl = await uploadReportFile(fileData, fileName || `report-${Date.now()}`)
    }

    // Update report fields
    report.name = name || report.name
    report.category = category || report.category
    report.date = date || report.date
    report.lab = lab !== undefined ? lab : report.lab
    report.doctor = doctor !== undefined ? doctor : report.doctor
    report.status = status || report.status
    report.fileUrl = fileUrl
    report.results = results || report.results
    report.notes = notes !== undefined ? notes : report.notes
    report.updatedAt = new Date()

    await patient.save()

    return NextResponse.json({ success: true, report }, { status: 200 })
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error.message }, { status: 500 })
  }
}

// DELETE a report
export async function DELETE(req: NextRequest) {
  try {
    await dbConnect()

    const authHeader = req.headers.get("authorization")
    const decoded = verifyAuth(authHeader)

    if (decoded.role !== "patient") {
      return NextResponse.json({ success: false, message: "Unauthorized access" }, { status: 403 })
    }

    const url = new URL(req.url)
    const reportId = url.searchParams.get("id")

    if (!reportId) {
      return NextResponse.json({ success: false, message: "Report ID is required" }, { status: 400 })
    }

    const updateResult = await Patient.updateOne(
      { userId: decoded.sub },
      { $pull: { reports: { _id: new mongoose.Types.ObjectId(reportId) } } }
    )

    if (updateResult.matchedCount === 0) {
      return NextResponse.json({ success: false, message: "Patient profile not found" }, { status: 404 })
    }

    if (updateResult.modifiedCount === 0) {
      return NextResponse.json({ success: false, message: "Report not found" }, { status: 404 })
    }

    return NextResponse.json({ success: true, message: "Report deleted successfully" }, { status: 200 })
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error.message }, { status: 500 })
  }
}
